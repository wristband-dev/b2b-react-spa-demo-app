...
export const getAssignableRoleOptions = async function (tenantId, requestConfig) {
  ...
  const response = await backendClientWithInterceptor.get(
    `/v1/tenants/${tenantId}/roles?${queryString}`,
    requestConfig
  );
...
export const getAssignedRole = async function (userId, requestConfig) {
  const response = await backendClientWithInterceptor.get(
    `/v1/users/${userId}/roles?fields=id,name,displayName&count=1`,
    requestConfig
  );
...
export const getUserSchemaForTenant = async function (tenantId, requestConfig) {
  const response = await backendClientWithInterceptor.post(
    `/v1/tenants/${tenantId}/user-schemas/resolve-overrides`,
    null,
    requestConfig
  );
  return response.data;
...
export const revokeRefreshToken = async function (token) {
  await backendClient.post(
    `/v1/oauth2/revoke`,
    createFormData({ token: token, client_id: CLIENT_ID }),
    BASIC_AUTH_AXIOS_CONFIG
  );
};
...
export const cancelEmailChange = async function (changeEmailRequestId, requestConfig) {
  await backendClientWithInterceptor.post(
    '/v1/change-email/cancel-email-change',
...
export const cancelNewUserInvite = async function (newUserInvitationRequestId, requestConfig) {
  await backendClientWithInterceptor.post(
    `/v1/new-user-invitation/cancel-invite`,
    { newUserInvitationRequestId },
    requestConfig
  );
};
...
export const getChangeEmailRequestsForUser = async function (userId, requestConfig) {
  ...
  const response = await backendClientWithInterceptor.get(
    `/v1/users/${userId}/change-email-requests?query=${statusQuery}`,
    requestConfig
  );
  return response.data;
};
...
export const getPermissionInfo = async function (values, requestConfig) {
  const response = await backendClientWithInterceptor.get(
    `/v1/permission-info?values=${values.join(',')}`,
    requestConfig
  );
  const { totalResults, items } = response.data;
...
export const updateTenant = async function (tenantId, tenantData, requestConfig) {
  const { invoiceEmail, displayName } = tenantData;
  const properTenantData = { displayName, publicMetadata: { invoiceEmail } };

  const tenantResponse = await backendClientWithInterceptor.patch(
    `/v1/tenants/${tenantId}`,
    properTenantData,
    requestConfig
  );
